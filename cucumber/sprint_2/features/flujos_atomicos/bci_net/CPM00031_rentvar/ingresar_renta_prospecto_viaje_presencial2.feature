...
Validaciones en la pantalla "Detalle su Perfil" (E2 Complementar Perfil)
...
@BCISAV-550
Escenario: BCISAV-550_Validar que en la etapa "Detalle su perfil" se visualice sub-etapa "Valide su renta", donde se despliegue:
...
@BCISAV-551
Escenario: BCISAV-551_Validar que en Seleccione el tipo de Ingreso se despliegue las siguientes elecciones:
...
@BCISAV-552
Escenario: BCISAV-552_Validar que al presionar link Ayuda en sub-etapa Valide su renta se despliegue PopUp con el siguiente mensaje:
...
@BCISAV-553
Escenario: BCISAV-353_Validar que en la parte inferior se visualice el siguiente mensaje:
...
@BCISAV-555
Escenario: BCISAV-555_Validar que Etapa código viaje se encuentre en "3" en Base de datos "Prospecto" en la tabla "Viaje" en el campo "etp_codigo".
  Cuando Selecciona tipo de ingreso variable bcinet seguimiento
...
@BCISAV-556
Escenario: BCISAV-556_Validar que viaje Estado se encuentre en "1" en Base de datos "Prospecto" en la tabla "Viaje" en el campo "vje_estado".
  Cuando Selecciona tipo de ingreso variable bcinet seguimiento
  Y Ingresar renta liquida mensual (1480000 pesos)
  Y Presiona Boton Continúe bcinet
  Dado Obtener datos tabla "viaje"
...
@BCISAV-544
Escenario: BCISAV-544_Validar Código Seguimiento en la tabla "ticket_atencion" en el campo "tck_atn_codigo"
  Cuando Selecciona tipo de ingreso variable bcinet seguimiento
  Y Ingresar renta liquida mensual (1480000 pesos)
  Y Presiona Boton Continúe bcinet
...
@BCISAV-559
Escenario: BCISAV-559_Validar que no se muestre en Combo Box de Nombre "Estado Civil", "Acuerdo de Unión Civil"
  Cuando Selecciona tipo de ingreso variable bcinet seguimiento
  Y Ingresar renta liquida mensual (1480000 pesos)
  Y Presiona Boton Continúe bcinet
